...
package net.mcreator.firefightersmod.entity;
...
import net.mcreator.firefightersmod.procedures.WaterProjectileHitsLivingEntityProcedure;
import net.mcreator.firefightersmod.procedures.WaterProjectileHitsBlockProcedure;
...
public class WaterEntity extends AbstractArrow implements ItemSupplier {
	...
	public WaterEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(FirefightersModModEntities.WATER.get(), world);
	}
	...
	public void tick() {
		super.tick();
		WaterProjectileHitsLivingEntityProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
		if (this.inGround)
			this.discard();
	}
	...
	public static WaterEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		WaterEntity entityarrow = new WaterEntity(FirefightersModModEntities.WATER.get(), entity, world);
	...
	public static WaterEntity shoot(LivingEntity entity, LivingEntity target) {
		WaterEntity entityarrow = new WaterEntity(FirefightersModModEntities.WATER.get(), entity, entity.level());
